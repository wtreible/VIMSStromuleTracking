...
if ischar(disposalmethod) && numframes>1
...
function writeaframe()
	if numframes==1
		if tfhastc
			imwrite(imind,cm,filepath,'gif','DelayTime',delay,'Loopcount',inf,'TransparentColor',tcidxthisframe,'DisposalMethod',disposalmethod);
		else
			imwrite(imind,cm,filepath,'gif','DelayTime',delay,'Loopcount',inf,'DisposalMethod',disposalmethod);
		end
	else
		if tfhastc
			if f==1
