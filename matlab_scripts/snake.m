function [snake_pnts,e,e2] = snake(pnts, alpha, beta, gamma, max_delta_x, resol_x, ...
...
% warranty disclaimers which appear in this software program.
  ...
  n = size(pnts,1);
  mean_dist = mean(sqrt(sum((pnts(2:end,:)-pnts(1:end-1,:)).^2,2)));
  ...
  for k=n-1:-1:1
    ...
    e2=e2+1-abs(target(states_i(k,v1)));
  end;
