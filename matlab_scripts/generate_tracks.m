...
% Input Params
data_dir = 'C:\Users\Toast\Desktop\Stromule_Track\input_data\';
output_directory = 'C:\Users\Toast\Desktop\Stromule_Track\output_data\';
input_ext = '.tif';
stromule_channel = 2;
...
% Mode flags
track_stromules = false;
correct_stromules = false;
generate_imgs = false;
    ...
    for fno=3:numel(lst)
        ...
        if track_stromules
            ...
            for imno=0:num_slices-1
                ...
                if imno == 0 % Initialize snakes on first frame
                    ...
                    all_stromules = evolve_snakes(I_s,new_stromules,10);
                else
                    all_stromules = evolve_snakes(I_s,all_stromules,10);
                    new_stromules = get_snake_pts(get_remaining(I_s,all_stromules));
                    all_stromules = [all_stromules, evolve_snakes(I_s,new_stromules,10)];
                end 
                ...
                disp(['  >> Timepoint Duration (s): ' num2str(iter_time)])
            end
        end
        %=====================%
        %%%%%%%%%%%%%%%%%%%%%%%
        ...
        if correct_stromules
            ...
            for tno=1:numel(tracks)
                disp(['  >> Analyzing Track: ', num2str(tno)])
                st = dlmread([tracks_dir, save_filename, '/', num2str(tno), '.txt']);
                xs = round(st(1:3:end,:));
                ys = round(st(2:3:end,:));
                zs = round(st(3:3:end,:));
